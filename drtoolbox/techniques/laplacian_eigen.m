...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    D = spdiags(sum(G, 2), 0, size(G,2), size(G,2));
    ...
    if strcmp(eig_impl, 'JDQR')
        ...
        [mappedX, lambda] = jdqr(L, D, no_dims + 1, tol, options);			% only need bottom (no_dims + 1) eigenvectors
    else
        options.disp = 0;
        options.isreal = 1;
        options.issym = 1;
    ...
    % Store data for out-of-sample extensions
    mapping.K = G;
    mapping.aff = G;
    mapping.vec = mappedX;
    mapping.val = lambda;

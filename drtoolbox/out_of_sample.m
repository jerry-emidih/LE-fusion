...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    switch mapping.name
        ...
        case 'Laplacian'         
            ...
            % Compute embeddings
            t_point = repmat(0, [size(point, 1) numel(mapping.val)]);
            K = (L2_distance(point', mapping.X') .^ 2)';
            for i=1:size(point, 1)
                
                % Compute Gaussian kernel between test point and training points
%                 K = (L2_distance(point(i,:)', mapping.X') .^ 2)';
%                 K = (pdist2(point(i,:), mapping.X) .^ 2)';
%                 K = K ./ mapping.max_dist;
                Ki = K(:, i);
                [foo, ind] = sort(Ki, 'ascend');            
                Ki(ind(mapping.k+1:end)) = 0;
%                 K(ind(floor(mapping.k / (5/6))+1:end)) = 0;
                Ki(Ki ~= 0) = exp(-Ki(Ki ~= 0) / (2 * mapping.sigma ^ 2));

                % Normalize kernel
                Ki = (1 ./ n) .* (Ki ./ sqrt(mean(Ki) .* mean(mapping.K, 2)));

                % Compute embedded point
                t_point(i,:) = sum(mapping.vec .* repmat(Ki, [1 size(mapping.vec, 2)]), 1);
            end
